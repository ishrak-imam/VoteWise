...
export const cryptoUtilState = atom({
  ...
  default: {
    generateMnemonic: (_?: GenerateMnemonicMessage['payload']) => {
      return Promise.resolve<GenerateMnemonicResultMessage['payload']>({
        mnemonic: '',
      });
    },
    validateMnemonic: (_: ValidateMnemonicMessage['payload']) => {
      return Promise.resolve<ValidateMnemonicResultMessage['payload']>({
        isValid: false,
        address: '',
      });
    },
    ...
    checkAddress: (_: CheckAddressMessage['payload']) => {
      return Promise.resolve<CheckAddressResultMessage['payload']>({
        isValid: false,
        reason: '',
      });
    },
...
export const keyringState = atom({
  ...
  default: {
    ...
    verifyCredentials: (_: VerifyCredentialsMessage['payload']) => {
      return Promise.resolve<VerifyCredentialsResultMessage['payload']>({
        valid: false,
      });
    },
  },
